...
收益递减的原因：方差减少量与1/m成正比，随着m增加，1/m的减少量变小
当m=30到50时：方差减少约0.0133(1-ρ)σ²，占初始方差的13.3%(1-ρ)/(0.0333+0.9667ρ)
与ρ的关系：当基学习器相关性ρ较高时，收益递减更明显
...
性价比(m) = [A(m) - A(1)] / [T(m) - T(1)] × Penalty(m)
其中：
Penalty(m) = 
⎧ m/10,               若 m < 10
⎨ 1,                  若 10 ≤ m ≤ 50
...
对比三个数据集的结果（乳腺癌、红酒、手写数字）：
   (1)为什么手写数字数据集需要更多树才能达到高性能？
特征空间复杂度：64维特征空间，需要更多树探索特征组合
类别数多：10个数字类别，决策边界复杂
样本量大：1797个样本，需要更多树稳定估计
特征相关性：像素特征高度相关，需要更多树降低相关性
噪声水平：手写数字有书写变异，需要集成平均噪声
Bagging方差：Var(avg) = σ²/m + (1-1/m)ρσ²
当特征相关性强时，ρ较大，需要更大的m来降低方差
当类别数多时，σ²较大，需要更大的m来降低方差

   (2)数据集复杂度（特征数、类别数、样本数）如何影响最优树数量？
数据集复杂度对树数量的影响：
复杂度维度   对树数量的影响   数学关系   原因
特征数↑	   树数量↑        m ∝ √p   需要覆盖更多特征组合
类别数↑     树数量↑        m ∝ √k	  多分类决策边界复杂
样本数↑	   树数量↑        m ∝ log(n) 	需要更多树稳定大样本估计
噪声水平↑    树数量↑   	m ∝ 1/(1-ρ)    需要平均噪声

   (3)请给出一个经验公式，根据数据集特征估算初始树数量
初始树数量 = 50 × √(特征数/10) × log10(样本数/100) × √(类别数/2)